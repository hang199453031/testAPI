			...
			<html xmlns="http://www.w3.org/1999/xhtml">
				...
				<body>
					...
					<table width="100%" cellpadding="5" cellspacing="1">
						...
						<xsl:for-each select="$suiteElements">
							<xsl:variable name="testCaseName2" select="./test/@name" />
							<!-- <xsl:value-of select="./test/@name" /> -->

							<xsl:variable name="testCaseClassElements" select="test/class" />
							<xsl:for-each select="$testCaseClassElements">
								<xsl:variable name="testCaseClassMethodName2" select="@name" />

									<xsl:variable name="methodCount"
										select="count(distinct-values(./test-method/@name))"></xsl:variable>

									<xsl:for-each select="distinct-values(./test-method/@name)">

										<xsl:variable name="ii" select="position()"></xsl:variable>

										<xsl:variable name="testCaseClassMethodElementsName"
											select="."></xsl:variable>
										<tr style="background-color: #f5f5f5; font-size: 12px">
											<td>
												<xsl:value-of select="$testCaseClassMethodName2" />
											</td>
											<td>
												<xsl:value-of select="." /> <!-- -<xsl:value-of select="$ii"></xsl:value-of> -->
											</td>
											<td style="background-color: lightgreen; padding: 3px 3px 3px 0;"
												align="center">
												<xsl:value-of
													select="count($testCaseClassElements/test-method[@status='PASS' and @name=$testCaseClassMethodElementsName])" />
											</td>
											<td style="background-color: #FFBBBB; padding: 3px 3px 3px 0;"
												align="center">
												<xsl:value-of
													select="count($testCaseClassElements/test-method[@status='FAIL' and @name=$testCaseClassMethodElementsName])" />
											</td>
											<td align="center">
												<xsl:value-of
													select="count($testCaseClassElements/test-method[@name=$testCaseClassMethodElementsName])" />
											</td>
										</tr>
									</xsl:for-each>
			...
			<tr>
				<td colspan="4" class="detailsBox">
					<div id="{$detailsId}" class="testMethodDetails">
						...
						<xsl:call-template name="formField">
							<xsl:with-param name="label" select="'log'" />
							<xsl:with-param name="value" select="reporter-output/line" />
						</xsl:call-template>	
						<xsl:if test="./params">
							<xsl:call-template name="formField">
